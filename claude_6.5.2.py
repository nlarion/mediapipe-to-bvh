...
class Pose2BVHConverter:
    ...
    def convert_sequence_to_bvh(self, pose_sequence: List[np.ndarray], fps: float = 30.0) -> bvhio.BvhJoint:
        ...
        # Create root joint - bvhio doesn't take parent in constructor
        root = bvhio.BvhJoint('Hips')
        root.offset = [0, 0, 0]
        root.children = []  # Initialize children list
        joints['Hips'] = root
        ...
        for joint_name in self.joint_names[1:]:
            ...
            # Create joint without parent parameter
            joint = bvhio.BvhJoint(joint_name)
            ...
            if not hasattr(parent_joint, 'children'):
                parent_joint.children = []
...
def process_video(video_path: str, output_path: str,  vitpose_model_path: str, yolo_model_path: str, visualize: bool = False, model_size: str = 'b'):
    ...
    try:
        # Try the standard write method
        with open(output_path, 'w') as f:
            bvhio.write_bvh(f, bvh_root)
    except AttributeError:
        # If write_bvh doesn't exist, try alternative methods
        try:
            # Some libraries use write() method on the root joint
            with open(output_path, 'w') as f:
                bvh_root.write(f)
        except AttributeError:
            # Last resort - try to save directly
            bvhio.write(output_path, bvh_root)
