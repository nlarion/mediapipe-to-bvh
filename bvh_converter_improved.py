...
class ImprovedBVHConverter:
    ...
    def convert(self, pose_frames: List[PoseFrame], output_path: str) -> bool:
        ...
        if self.enable_ik:
            ...
            print("Applying improved IK foot locking...")
        ...
        # Calculate hip positions with drift correction
    ...
    def _initialize_improved_ik_system(self, reference_landmarks, scale: float):
        ...
        # NEW: Establish ground level from reference frame
        ...
        # These values are tuned for typical human walking speeds

        # Velocity threshold: ~0.5 m/s walking speed in real world
        self.ik_system.config.contact_velocity_threshold = 0.5 * (scale / 100.0)

        # Height threshold: foot is on ground if within 5cm of ground level
        self.ik_system.config.contact_height_threshold = 0.05 * scale

        # Foot clearance: minimum height for foot to be considered off ground
        self.ik_system.config.foot_clearance_height = 0.03 * scale
    ...
    def _apply_improved_ik_corrections(self, all_leg_positions: List[Optional[Dict]]) -> List[Optional[Dict]]:
        ...
        for i, leg_positions in enumerate(tqdm(all_leg_positions, desc="Applying IK")):
            ...
            # Override IK confidence with our improved detection
            if left_contact:
                ik_result['left']['confidence'] = 1.0
            if right_contact:
                ik_result['right']['confidence'] = 1.0
    ...
    def _detect_foot_contact(self, ankle_pos: np.ndarray, foot_pos: np.ndarray,
        ...
        foot_height = foot_pos[1] - self.ground_level if self.ground_level else foot_pos[1]
        ankle_height = ankle_pos[1] - self.ground_level if self.ground_level else ankle_pos[1]

        # Foot is on ground if close to ground level
        height_contact = min(foot_height, ankle_height) < self.ik_system.config.contact_height_threshold

        # Check velocity if we have previous frame
        velocity_contact = True
        if prev_ankle is not None:
            ankle_velocity = np.linalg.norm(ankle_pos - prev_ankle)
            velocity_contact = ankle_velocity < self.ik_system.config.contact_velocity_threshold

        # Both conditions should be met for solid contact
        return height_contact and velocity_contact
    ...
    def _calculate_frame_rotations_improved(self, landmarks,
        ...
        def process_joint(joint: BVHJoint):
            ...
            if joint.name == "Head":
                rotations[joint.name] = np.array([5.0, 0.0, 0.0])
    ...
    def _calculate_hip_positions_with_drift_correction(self, pose_frames: List[PoseFrame]) -> List[np.ndarray]:
        """Calculate hip positions with foot-based drift correction."""
        positions = []
        ...
        for i, frame in enumerate(pose_frames):
            if frame.world_landmarks:
                ...
                hip_center = np.array([
                    ...
                    -(left_hip.y + right_hip.y) / 2 * self.scale,
        ...
        # Apply temporal smoothing with Y-axis preservation
