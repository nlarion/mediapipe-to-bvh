...
try:
    import bvhio
    from bvhio import Joint, Transform
except ImportError:
...
class Pose2BVHConverter:
    ...
    def convert_sequence_to_bvh(self, pose_sequence: List[np.ndarray], fps: float = 30.0) -> Tuple[bvhio.Joint, float]:
        """
        Convert a sequence of 3D poses to BVH format using bvhio
        ...
        Returns:
            BVH root joint
        """
        # Get first frame to calculate offsets
        first_frame = pose_sequence[0]
        
        # Create skeleton hierarchy using bvhio.Joint
        joints = {}
        
        # Create root joint with proper Transform
        root_pos = self._get_joint_position('Hips', first_frame)
        root_transform = bvhio.Transform(position=root_pos.tolist())
        root = bvhio.Joint('Hips', restPose=root_transform)
        joints['Hips'] = root
        
        # Track which joints have end sites (leaf nodes)
        children_map = {}
        ...
        # Create other joints recursively
        def create_joint_hierarchy(parent_joint, parent_name):
            if parent_name in children_map:
                for child_name in children_map[parent_name]:
                    # Calculate offset from parent
                    parent_pos = self._get_joint_position(parent_name, first_frame)
                    child_pos = self._get_joint_position(child_name, first_frame)
                    offset = child_pos - parent_pos
                    
                    # Create child joint
                    child_transform = bvhio.Transform(position=offset.tolist())
                    child_joint = bvhio.Joint(child_name, restPose=child_transform)
                    
                    # Attach to parent
                    parent_joint.attach(child_joint)
                    joints[child_name] = child_joint
                    
                    # Recursively create children
                    create_joint_hierarchy(child_joint, child_name)
            else:
                # Add end site for leaf joints
                # Estimate end site position based on parent direction
                if parent_name in self.hierarchy and self.hierarchy[parent_name]:
                    grandparent_name = self.hierarchy[parent_name]
                    grandparent_pos = self._get_joint_position(grandparent_name, first_frame)
                    parent_pos = self._get_joint_position(parent_name, first_frame)
                    direction = parent_pos - grandparent_pos
                    direction_norm = direction / (np.linalg.norm(direction) + 1e-8)
                    end_offset = direction_norm * 0.1  # Small offset for end site
                else:
                    end_offset = np.array([0, -0.1, 0])  # Default down
                
                end_transform = bvhio.Transform(position=end_offset.tolist())
                end_site = bvhio.Joint('End Site', restPose=end_transform)
                parent_joint.attach(end_site)
        
        # Build hierarchy starting from root
        create_joint_hierarchy(root, 'Hips')
        
        # Process animation data
        # For each joint, create keyframes
        for joint_name, joint in joints.items():
            ...
            for frame_idx, keypoints_3d in enumerate(pose_sequence):
                if joint_name == 'Hips':
                    ...
                    # For now, use identity rotation for root
                    transform = bvhio.Transform(
                        position=hip_pos.tolist(),
                        rotation=(0, 0, 0)
                    )
                else:
                    ...
                    # Find child for bone vector
                    if joint_name in children_map and children_map[joint_name]:
                        # Use first child
                        child_pos = self._get_joint_position(children_map[joint_name][0], keypoints_3d)
                    else:
                    ...
                    transform = bvhio.Transform(rotation=(z_rot, x_rot, y_rot))
                
                # Add keyframe (frame_number, transform)
                keyframes.append((frame_idx, transform))
            
            # Set keyframes for joint
            joint.Keyframes = keyframes
...
def process_video(video_path: str, output_path: str,  vitpose_model_path: str, yolo_model_path: str, visualize: bool = False, model_size: str = 'b'):
    ...
    bvh_root, frame_time = converter.convert_sequence_to_bvh(pose_sequence_3d, fps)
    
    # Save BVH file
    print(f"Saving BVH to: {output_path}")
    try:
        # Use bvhio.writeHierarchy to save
        bvhio.writeHierarchy(output_path, bvh_root, frame_time, percision=6)
    except Exception as e:
        print(f"Error saving BVH: {e}")
        # Try alternative spelling
        try:
            bvhio.writeHierarchy(output_path, bvh_root, frame_time, precision=6)
        except:
            # Try without precision parameter
            bvhio.writeHierarchy(output_path, bvh_root, frame_time)
...
def main():
    ...
    if args.output is None:
        base_name = os.path.splitext(os.path.basename(args.video))[0]  # Fixed: was args.input
