...
def get_joint_mapping():
    ...
    return {
        ...
        # Enhanced leg mappings with more comprehensive landmark sets
...
def get_default_bone_offsets():
    ...
    return {
        ...
        # Improved leg and foot offsets for better anatomical correctness
        "LeftUpLeg": np.array([0.1, -0.1, 0.0]),       # Down left from hips
        "LeftLeg": np.array([0.0, -0.3, 0.02]),        # Down from left hip with slight forward bend
        "LeftFoot": np.array([0.0, -0.28, 0.05]),      # Down and forward from left knee
        "LeftToeBase": np.array([0.0, -0.02, 0.12]),   # Forward and slightly down from left ankle

        "RightUpLeg": np.array([-0.1, -0.1, 0.0]),     # Down right from hips
        "RightLeg": np.array([0.0, -0.3, 0.02]),       # Down from right hip with slight forward bend
        "RightFoot": np.array([0.0, -0.28, 0.05]),     # Down and forward from right knee
        "RightToeBase": np.array([0.0, -0.02, 0.12])   # Forward and slightly down from right ankle
    }
    ...
    def process_joint(joint):
        ...
        if joint.parent:
            ...
            if joint_pos is not None and parent_pos is not None:
                ...
                # Enhanced handling for leg and foot joints
                if joint.name in ["LeftUpLeg", "RightUpLeg"]:
                    # Ensure proper hip joint orientation
                    if "Left" in joint.name:
                        # Ensure left hip has positive X component (extends left from center)
                        if offset[0] < 0.05:
                            offset[0] = 0.1  # Minimum X offset for left hip
                    else:  # Right hip
                        # Ensure right hip has negative X component (extends right from center)
                        if offset[0] > -0.05:
                            offset[0] = -0.1  # Minimum X offset for right hip

                    # Add consistent downward component
                    offset[1] = -abs(offset[1]) if offset[1] != 0 else -0.1

                elif joint.name in ["LeftLeg", "RightLeg"]:
                    # Ensure knees have proper orientation - should go downward and slightly forward
                    offset[1] = -abs(offset[1]) if offset[1] != 0 else -0.25  # Ensure negative Y (downward)
                    if abs(offset[2]) < 0.03:  # If almost no forward component
                        offset[2] = 0.03  # Add small forward component

                elif joint.name in ["LeftFoot", "RightFoot"]:
                    # Feet should extend down from knees and forward
                    offset[1] = -abs(offset[1]) if offset[1] != 0 else -0.25  # Ensure negative Y (downward)
                    if offset[2] < 0 and abs(offset[2]) > 0.05:  # If pointing significantly backward
                        offset[2] = abs(offset[2])  # Flip direction to point forward
                    elif abs(offset[2]) < 0.05:  # If almost no forward/backward component
                        offset[2] = 0.05  # Add small forward component

                elif joint.name in ["LeftToeBase", "RightToeBase"]:
                    # Toes should point forward and slightly down from ankle
                    if offset[2] < 0.05:  # If not pointing forward enough
                        offset[2] = 0.1  # Set minimum forward component
                    if offset[1] > 0:  # If pointing upward
                        offset[1] = -0.02  # Slight downward component
        ...
        if joint.name == "Head":
            ...
            if np.linalg.norm(joint.offset) < 5.0:
                joint.offset = np.array([0.0, -8.0, 3.0])  # Down and forward with increased forward component
            ...
            if np.linalg.norm(joint.offset) < 5.0:
                joint.offset = np.array([0.0, -2.0, 10.0])  # Forward with increased forward component
...
def calculate_frame_rotations(frame_landmarks, joint_mapping, skeleton, default_rotations):
    ...
    # Helper function to get parent-child-grandchild plane for better knee/elbow orientation
    def get_bend_plane_normal(parent_name, joint_name, child_name):
        parent_pos = get_joint_position(parent_name, frame_landmarks, joint_mapping)
        joint_pos = get_joint_position(joint_name, frame_landmarks, joint_mapping)
        child_pos = get_joint_position(child_name, frame_landmarks, joint_mapping)

        if parent_pos is not None and joint_pos is not None and child_pos is not None:
            v1 = joint_pos - parent_pos  # Vector from parent to joint
            v2 = child_pos - joint_pos   # Vector from joint to child

            # Calculate normal to the plane formed by these vectors
            ...
            for child in joint.children:
                ...
                if direction is not None:
                    ...
                    if rest_direction is not None:
                        # Enhanced corrections for leg and foot joints
                        # UpLeg joints (hips to knees)
                        if joint.name in ["LeftUpLeg", "RightUpLeg"]:
                            # For leg joints, we need to handle rotation more carefully
                            # Hips should mainly rotate around X axis (for forward/backward) and Y axis for sideways
                            # Get a plane normal from hip-knee-ankle to determine twist
                            grandchild_name = "LeftFoot" if joint.name == "LeftUpLeg" else "RightFoot"
                            plane_normal = get_bend_plane_normal("Hips", child.name, grandchild_name)

                            if plane_normal is not None and plane_normal[0] != 0:
                                # Ensure proper left/right orientation
                                expected_sign = -1 if joint.name == "LeftUpLeg" else 1
                                if np.sign(plane_normal[0]) != expected_sign:
                                    plane_normal = -plane_normal

                            # Make sure legs go down
                            if direction[1] > 0:  # If pointing up
                                direction[1] = -abs(direction[1])

                        # Leg joints (knees)
                        elif joint.name in ["LeftLeg", "RightLeg"]:
                            # Knees should mainly bend around X axis (forward/backward)
                            # Make sure the knee bends forward
                            if direction[2] < 0:  # If backward
                                direction[2] = abs(direction[2]) * 0.1  # Point slightly forward

                            # Ensure consistent downward direction
                            if direction[1] > 0:  # If pointing up
                                direction[1] = -abs(direction[1])

                        # Foot joints (ankles)
                        elif joint.name in ["LeftFoot", "RightFoot"]:
                            # Ankle should create proper foot orientation
                            # Ensure forward orientation with proper sideways alignment
                            if direction[2] < 0:  # If pointing backward
                                direction[2] = abs(direction[2])

                            # Prevent extreme rotations
                            if abs(direction[0]) > 0.3:  # If foot rolls too much sideways
                                direction[0] = np.sign(direction[0]) * 0.3

                        # Toe joints
                        elif joint.name in ["LeftToeBase", "RightToeBase"]:
                            # Toes should point forward with slight downward angle
                            if direction[2] < 0.7:  # If not pointing predominantly forward
                                # Normalize the forward component while preserving some of the original direction
                                orig_dir = np.copy(direction)
                                direction = np.array([direction[0] * 0.3, -0.1, 0.8])  # Mostly forward, slightly down
                                direction = direction / np.linalg.norm(direction)  # Normalize

                        # Regular handling for other joints
                        elif joint.name in ["Head"]:
                            # For head, ensure it's not tilted extremely
                            if direction[1] < 0:  # If pointing down
                                direction[1] = abs(direction[1])

                        # For hands, ensure they extend out from arms correctly
                        elif joint.name == "LeftHand" and direction[0] > 0:
                        ...
                        if np.linalg.norm(cross) > 1e-10:
                            ...
                            # Apply specific adjustments for improved rotation
                            if joint.name in ["LeftUpLeg", "RightUpLeg"]:
                                # Limit extreme hip rotations
                                for i in range(3):
                                    if abs(euler_angles[i]) > 120:
                                        euler_angles[i] = np.sign(euler_angles[i]) * 120

                            elif joint.name in ["LeftLeg", "RightLeg"]:
                                # Make knee rotation primarily around X-axis (anatomically correct)
                                euler_angles[1] *= 0.3  # Reduce Y rotation (twist)
                                euler_angles[2] *= 0.3  # Reduce Z rotation (side bend)

                                # Ensure knees bend in correct direction
                                if euler_angles[0] < 0:  # Knees should bend forward (positive X rotation)
                                    euler_angles[0] = abs(euler_angles[0])

                            elif joint.name in ["LeftFoot", "RightFoot"]:
                                # Prevent extreme ankle rotations
                                if abs(euler_angles[0]) > 45:  # X rotation (up/down tilt)
                                    euler_angles[0] = np.sign(euler_angles[0]) * 45
                                if abs(euler_angles[2]) > 25:  # Z rotation (roll)
                                    euler_angles[2] = np.sign(euler_angles[2]) * 25
...
def write_joint_hierarchy(f, joint, indent_level):
    ...
    if not joint.children:
        ...
        # Use anatomically appropriate end site offsets with improved foot/toe positioning
        if joint.name == "Head":
            ...
            f.write(f"{indent}    OFFSET 3.0 0.0 2.0\n")
        elif joint.name == "LeftToeBase":
            # Left toe end site points forward and slightly downward
            f.write(f"{indent}    OFFSET 0.0 -1.0 5.0\n")
        elif joint.name == "RightToeBase":
            # Right toe end site points forward and slightly downward
            f.write(f"{indent}    OFFSET 0.0 -1.0 5.0\n")
        else:
            # Default end site
