def parse_bvh(file_path):
    ...
    # More robust parsing of frame count and frame time
    for line in motion_lines[:3]:  # Check the first few lines
        line = line.strip()
    ...
    if num_frames is None or frame_time is None:
        ...
        print(f"First lines of motion section: {motion_lines[:3]}")
        return None, None, None, None
    
    # Find where the motion data starts (after header)
    data_start_idx = 0
...
def preview_bvh_animation(bvh_file, speed_factor=1.0):
    ...
    try:
        ...
        # Parse the BVH file
        joints, joint_order, motion_data, frame_time = parse_bvh(bvh_file)
        if None in (joints, joint_order, motion_data, frame_time):
            print("Failed to parse BVH file. Cannot preview animation.")
            return
...
def build_skeleton_from_landmarks(frame_landmarks, joint_mapping, scale=100.0):
    ...
    # Debug the landmark positions
    debug_landmarks = False
    if debug_landmarks:
        print("\nLandmark Positions:")
        for name, indices in joint_mapping.items():
            positions = []
            for idx in indices:
                pos = get_landmark_position(frame_landmarks, idx)
                if pos is not None:
                    positions.append(pos)
            
            if positions:
                avg_pos = np.mean(positions, axis=0)
                print(f"{name}: {avg_pos}")
