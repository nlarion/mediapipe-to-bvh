...
It combines the strengths of two different implementations:
- Accurate global position tracking from claude.py
- Better limb pose estimations from bvh9.py

Features:
- Accurate global position tracking for walking/moving characters
- High-quality limb rotation calculations
...
Usage: python combined_mediapipe_bvh.py --video input.mp4 --output motion.bvh [options]
"""
...
def calculate_global_position_delta(current_positions, previous_positions, frame_width, frame_height):
    ...
    Returns:
        delta: 3D vector of position change in world space
    """
...
def calculate_frame_rotations(frame_landmarks, joint_mapping, skeleton, default_rotations):
    """Calculate rotations for all joints based on the current frame landmarks using bvh9's approach"""
...
def process_landmarks_to_skeleton(landmarks, skeleton, frame_idx, first_frame=False, 
    ...
    Combines bvh9's limb pose estimation with claude.py's global position tracking.
    """
    ...
    default_rotations = {joint_name: np.zeros(3) for joint_name in skeleton.joints}
    
    # Calculate and update joint rotations using bvh9's approach
    frame_rotations = calculate_frame_rotations(landmarks, joint_mapping, skeleton, default_rotations)
    
    # Update the rotations for each joint
    for joint_name, rotation in frame_rotations.items():
